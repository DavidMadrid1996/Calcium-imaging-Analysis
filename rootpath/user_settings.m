%% David Madrid . Last Rev 12/09/2022 
...
SNAP_directory = fullfile(path.data,'SNAP'); % directory for the high resolution snap of that trial
zstack_directory = fullfile(path.data,'zstack'); % directory for the zstack
...
a4=dir([Videoreal_directory  '/*.avi']); videoreal_num=length(a4);
a5=dir([CZI_directory '/*.czi']); czi_num=length(a5);
a6=dir([SNAP_directory '/*.czi']); snap_num=length(a6);

% equality_matrix = [czi_num csv_num videocsv_num videoreal_num ];
equality_matrix = [czi_num  csv_num videoreal_num ]; % snap_num 
...
for i = 1:length(A_cell)
 x = split(A_cell(i,1),".avi");
 list3{i,1} = x{1}; % Creating list3 with the name of the videos
end 

A_cell = struct2cell(a6); % Spliting czi
A_cell = A_cell(1,:)';
list4 = strings(length(A_cell),1);

% for i = 1:length(A_cell)
%  x = split(A_cell(i,1),".czi");
%  list4{i,1} = x{1}; % Creating list3 with the name of the snaps
% end 



if all(list == list2) && all(list == list3) %&&  all(list == list4)
...
A_cell = struct2cell(a6); 
A_cell = A_cell(1,:)';
list4 = strings(length(A_cell),1);

for i = 1:length(A_cell)
 list4(i,1) = A_cell(i,1); 
end 


% Clearing variables 

clear a2 a4 a5 a6 A_cell equality_matrix
