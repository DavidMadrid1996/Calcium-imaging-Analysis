...
%% Cuting swimming traces and the correspondence calcium traces
% Used in case the activity is not clear or not analyzable

% 1.Plot for visual checking
plot(CALCIUMroiTS.deeplabcut.time_scaled,wave) 
pause(0.5)
...
if isequal(answer,'Y')
    ...
    CALCIUM.SumAng3D = CALCIUM.SumAng3D(xi_swim_cutted:xf_swim_cutted);
    CALCIUM. AngHeadTail3D = CALCIUM. AngHeadTail3D(xi_swim_cutted:xf_swim_cutted);
    ...
    plot(CALCIUMroiTS.deeplabcut.lasertimesync, wave)
    hold on
...
findpeaks(wave, 'MinPeakProminence',sidepeaks);
%xline(laserframeON)
%xline(laserframeOFF)
...
figure('units','normalized','outerposition',[0 0 1 1])
findpeaks(-wave, 'MinPeakProminence',sidetroughts);
%xline(laserframeON)
%xline(laserframeOFF)
...
figure('units','normalized','outerposition',[0 0 1 1])
findpeaks(wave, 'MinPeakProminence',sidepeaks); hold on
[pks1,locs1] = findpeaks(wave, 'MinPeakProminence',sidepeaks);
%xline(laserframeON)
%xline(laserframeOFF)
...
figure('units','normalized','outerposition',[0 0 1 1])
findpeaks(-wave, 'MinPeakProminence',sidetroughts); hold on
[pks2,locs2] = findpeaks(-wave, 'MinPeakProminence',sidetroughts);
%xline(laserframeON)
%xline(laserframeOFF)
...
% 
% error % fix this. You have to remove all the detections that are not good. 
% 
...
%         temp_mantain =  find(time_vector_sing(i) == CALCIUM.locs1_swim);
%         sign_retention (i) = CALCIUM.pks1_swim(temp_mantain);
% 
% 
%     elseif any(time_vector_sing(i) == CALCIUM.locs2_swim)
%         temp_mantain =  find(time_vector_sing(i) == CALCIUM.locs2_swim);
...
for i=1:length(x_time_B)/2
    baseline_swimming (i)=...
        mean( wave([xi_baseline_swim(1+sum):xi_baseline_swim(2+sum)])); %avrg

    std_baseline (i) = std( wave([xi_baseline_swim(1+sum):xi_baseline_swim(2+sum)])); %std
    sum = sum + 2;
end
...
for k=1:2:length(x_time)
    ...
    EPISODE_Y.(my_field2) =  wave(EPISODE_X.(my_field1));
...
%% MANUALLY LABEL FOR THE SWIMMING OR STRUGLE.

LabellingBehaviour = cell(1,length(fieldnames(CALCIUM.EPISODE_X )));

for j = 1:length(fieldnames(CALCIUM.EPISODE_X ))
    temp = fieldnames(CALCIUM.EPISODE_X );
    myfield = char(temp (j));

    Ini = CALCIUM.EPISODE_X.(myfield)(1)-20;
    Fin = CALCIUM.EPISODE_X.(myfield)(end)+20;

    figure('units','normalized','outerposition',[0 0 1 1])
    plot( wave(Ini:Fin)), hold on
    plot(1:length( wave(Ini:Fin)),CALCIUM.BASELINE_PER_EPISODE.(myfield)*ones(1,length( wave(Ini:Fin))))
    grid on
    title('Manual segmentation (labelling) of the episodes. Which is the behaviour?')
    pause(0.2)

    prompt = 'Is this episode swimming, turning or strugle? ';
    str = input(prompt,'s');

    LabellingBehaviour{j} = str;
    close all
end

CALCIUM.LABELLINGBH = LabellingBehaviour;
...
std_baseline = std( wave([xi_baseline_swim(1):xi_baseline_swim(2)])); %std

%Visual test
plot(CALCIUMroiTS.deeplabcut.lasertimesync, wave); hold on
...
for i=1:length(fieldnames(CALCIUM.EPISODE_Y)) %Loop across all the swimming episodes
    ...
    EPISODE_Y_BASELINE_AMP.(my_field) = CALCIUM.EPISODE_Y.(my_field)-tempor.(my_field); % we create
...
for i = 1:length(fieldnames(CALCIUM.EPISODE_X))
    ...
    for ii = 1:numer
%         idx = find(CALCIUM.EPISODE_X.(temp_sing)(ii)==time_vector_sing)
if EPISODE_Y_BASELINE_AMP.(temp_sing)(ii)<0
     CALCIUM.EPISODE_Y_BASELINE_AMP_SIGN.(temp_sing)(ii) = -1; % THIS IS RIGHT FOR DORSAL SIDE UP
elseif EPISODE_Y_BASELINE_AMP.(temp_sing)(ii)>0 ; 
    CALCIUM.EPISODE_Y_BASELINE_AMP_SIGN.(temp_sing)(ii) = 1; % THIS IS LEFT FOR DORSAL SIDE UP
end 
...
for k=1:length(fieldnames(CALCIUM.EPISODE_X)) % we will do it across swimming episodes
    ...
    CALCIUM.GENERAL_FREQ_Y.(myfield1) = freq./2; 
    end 
    ...
    if isempty(v1) % If there is no swimming there will be no index and all the rest of the code will give
        ...
        % REMOVE JUST TAILBEATING (THIS WILL EXCLUDE EVERYTHING THAT IS NOT CROSSING THE MIDLINE,
        % THIS IS, EVERYTHING IS NOT PROPER ALTERNATING SWIMMING)



        tail_beating=zeros(1,length(timepoints));
        tail_beating(1)=1;
        std_used = 10; % how many standars deviation above and below the baseline

        % Remove the values that are consecutivetly way below the baseline
        baseline = tempor.(myfield1);
        topbaseline = baseline + std_used*CALCIUM.std_baseline_swimming;
        bottombaseline = baseline - std_used*CALCIUM.std_baseline_swimming;
        yline(topbaseline)
        yline(bottombaseline)
        for  i=1:length(timepoints)-1
            if CALCIUM.EPISODE_Y.(myfield1)(i)<bottombaseline && CALCIUM.EPISODE_Y.(myfield1)(i+1)>topbaseline
                if tail_beating(i)==1
                    tail_beating(i+1)=1;
                else
                    tail_beating(i)=1;
            ...
            elseif CALCIUM.EPISODE_Y.(myfield1)(i)>topbaseline && CALCIUM.EPISODE_Y.(myfield1)(i+1)<bottombaseline
                if tail_beating(i)==1
                    tail_beating(i+1)=1;
                else
                    tail_beating(i)=1;
        ...
        % if the code above have some value of tail_beating==0 means
        % that there is some tail beating

        if size(find(tail_beating==0)) == [1 0] % if true means that there is not
            % tail beating

            v3= find(tail_beating); % if is swmming all of the value will be 1
        ...
        else % if is not true, mean that there is at least one zero so there is
            % at least one episode of tail beating. So we can calculate
            % now the frequency for the tail beating
            tail_beating(find(tail_beating==0)) = 1;
            v3= find(tail_beating); % if is swmming all of the value will be 1
            xline(timepoints(v3)*(sr),'b--','LineWidth',1)
            title(strcat(CALCIUM.LABELLINGBH{k},' - ',' Bypassing Tail beating'))
            jj = intersect(ii, v3);
        ...
        if length(datapoints)==1 %means there is no swimming
            ...
            xline(datapoints(v5),'m--','LineWidth',5)
...
% %% All variables important are:
% % CALCIUM.EPISODE_Y_BASELINE_AMP CALCIUM.BASELINE_PER_EPISODE CALCIUM.FREQ_SWIM_Y
%
