...
iiii=8; % Number of the fish to load (this is the trial for the same fish)
...
% color=['b'; 'r']
% [21 20 19 18 22 38 39 25 24 27 26 23 28 29 30 31 32 34 36 33 35 37]
for i = [3 17 16 4 5 15 14 13 6 12 11 7 8 9 10]


    yyaxis left
    r = -CALCIUMroiTS.diff_perc03.data(:,i);
% figure(i)
% TimeCameraRescale = (1:length(CALCIUMroiTS.deeplabcut.laserframe))./length(CALCIUMroiTS.deeplabcut.laserframe);
% plot(TimeCameraRescale,wave,'k','LineWidth',2); hold on
% yyaxis right
TimeCalciumRescale  = (1:length(CALCIUMroiTS.diff_perc03.times))./length(CALCIUMroiTS.diff_perc03.times);
plot(TimeCalciumRescale,r,'b-','LineWidth',2); hold on
...
%% Ploting acording to rostro caudal
clear PercentageActive Active
RostroCaudal = CALCIUM.ZCoordinates(:,2); 
Spaced=linspace(min(RostroCaudal),max(RostroCaudal),10); 
for i = 1:length(Spaced)-1
    tempor = find(RostroCaudal>Spaced(i) & RostroCaudal<Spaced(i+1)); 
    if isempty(tempor)
        PercentageActive(i) = 0
    else 
        count = 1;
        for ii = 1:length(tempor)
            my_field = strcat('CELL',num2str(tempor(ii))); % Creating dinamicly the
            if CALCIUM.BASELINE_TO_PEAK.(my_field)==0
                Active(count) = 0
                count = count + 1;
            else
                Active(count) = 1
        ...
        PercentageActive(i) = ((sum(Active))/length(tempor))*100; 

    end
    clear Active
end 

% area(PercentageActive,(Spaced(1:end-1))./max(Spaced)),hold on
plot(PercentageActive,(Spaced(1:end-1))./max(Spaced))
xlabel('Percentage of active cells')
CALCIUM. PercentageActive
